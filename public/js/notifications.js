...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // إذا كنا في الصفحة الرئيسية للنظام أو صفحة أخرى
    initializeUnreadCountIndicator();
});
...
/**
 * إعادة الاشتراك في إشعارات الويب
 * هذه الدالة تستخدم عندما نريد إجبار إعادة الاشتراك
 */
function forceResubscribeNotifications() {
    const resetButton = document.getElementById('reset-notifications-btn');
    if (resetButton) {
        resetButton.disabled = true;
        resetButton.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i> جاري إعادة الاشتراك...';
    }

    // إلغاء الاشتراك القديم من الخادم
    ...
    .then(() => {
        console.log('تم إلغاء الاشتراك القديم من الخادم');
        
        // مسح إصدار الإشعارات المخزن محليًا
        localStorage.removeItem('pushVersion');
        
        // إلغاء اشتراك المتصفح
        if ('serviceWorker' in navigator) {
            navigator.serviceWorker.ready
                ...
                .then(() => {
                    console.log('تم إلغاء اشتراك المتصفح بنجاح');
                    
                    // تحديث الصفحة لإعادة تشغيل عملية التسجيل مرة أخرى
                    showNotification('success', 'تم إعادة ضبط إشعارات الويب بنجاح. سيتم تحديث الصفحة الآن...');
                ...
                .catch(error => {
                    console.error('خطأ في إعادة ضبط إشعارات الويب:', error);
                    showNotification('error', 'حدث خطأ أثناء إعادة ضبط إشعارات الويب. يرجى المحاولة مرة أخرى.');
                    
                    if (resetButton) {
                        resetButton.disabled = false;
                        resetButton.innerHTML = '<i class="fas fa-sync-alt"></i> إعادة ضبط إشعارات الويب';
                    }
                });
        }
    })
    .catch(error => {
        console.error('خطأ في إلغاء الاشتراك من الخادم:', error);
        showNotification('error', 'حدث خطأ أثناء الاتصال بالخادم. يرجى المحاولة مرة أخرى.');
...
/**
 * عرض زر إعادة ضبط الإشعارات في صفحة الإشعارات
 */
function initializeNotificationsResetButton() {
    const controlsContainer = document.querySelector('.notifications-controls');
    
    if (controlsContainer) {
        // إنشاء الزر
        const resetButton = document.createElement('button');
        resetButton.id = 'reset-notifications-btn';
        resetButton.className = 'btn btn-secondary btn-sm ms-2';
        resetButton.innerHTML = '<i class="fas fa-sync-alt"></i> إعادة ضبط إشعارات الويب';
        resetButton.addEventListener('click', forceResubscribeNotifications);
...
// استدعاء تهيئة زر إعادة الضبط عند تحميل صفحة الإشعارات
document.addEventListener('DOMContentLoaded', function() {
    // تهيئة صفحة الإشعارات إذا كنا في صفحة الإشعارات
    if (window.location.pathname === '/notifications') {
        initializeNotificationsPage();
        initializeNotificationsResetButton();
    }
