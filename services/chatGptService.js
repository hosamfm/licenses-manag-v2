...
class ChatGptService {
  constructor() {
    ...
    // تعليمات إدارة المندوبين وتحويل المحادثات (سيتم تحميلها من قاعدة البيانات)
  ...
  async loadSettings() {
    try {
      ...
      if (settings) {
        ...
        // تحميل تعليمات تحويل المحادثات
        this.transferInstructions = settings.transferInstructions;
  ...
  async buildSystemMessage(customerInfo, previousConversations) {
    ...
    try {
      ...
      if (agents && agents.length > 0) {
        ...
        // استخدام تعليمات إدارة المندوبين وتحويل المحادثات من الإعدادات إذا كانت متاحة
        if (this.transferInstructions && this.transferInstructions.trim()) {
          systemMessage += `\n\n${this.transferInstructions}`;
          logger.debug('chatGptService', 'تم استخدام تعليمات تحويل المحادثات من الإعدادات', {
            instructionsLength: this.transferInstructions.length
          });
        } else {
          // في حالة عدم وجود تعليمات مخصصة، نضيف رسالة تنبيه للمدير
          systemMessage += `\n\nلم يتم تعيين تعليمات التحويل. يرجى استخدام علامة [تحويل:اسم_المندوب] لتحويل المحادثة إلى أحد المندوبين المتاحين.`;
          
          logger.warn('chatGptService', 'لم يتم العثور على تعليمات التحويل في قاعدة البيانات');
        }
      }
    } catch (error) {
