...
10. لا تخرج عن نطاق العمل إلا بشكل بسيط جدًا عند الحاجة لكسر الجمود، ثم عُد بسرعة لجو الخدمة والعمل.`;
  ...
  /**
   * تنسيق الرسائل لإرسالها إلى ChatGPT
   * @param {Array} messages سجل الرسائل
   * @param {String} systemMessage رسالة النظام
   * @returns {Array} مصفوفة الرسائل المنسقة
   */
  formatMessagesForChatGPT(messages, systemMessage) {
    try {
      // مصفوفة الرسائل المنسقة
      const formattedMessages = [];
      ...
      // تحويل كل رسالة إلى التنسيق المناسب
      ...
      // إذا كان عدد الرسائل أكبر من الحد الأقصى، نحتفظ فقط بأحدث الرسائل
      if (formattedMessages.length > this.maxMessagesCount) {
        // الاحتفاظ دائماً برسالة النظام (الأولى)
        const systemMessage = formattedMessages[0];
        // أخذ أحدث الرسائل بعد رسالة النظام
        const recentMessages = formattedMessages.slice(-(this.maxMessagesCount - 1));
        
        // إعادة بناء المصفوفة
        formattedMessages.length = 0;
        formattedMessages.push(systemMessage, ...recentMessages);
      }
      
      return formattedMessages;
    } catch (error) {
      logger.error('chatGptService', 'خطأ في تنسيق الرسائل لـ ChatGPT:', error);
      // إرجاع مصفوفة بسيطة في حالة الخطأ
      return [
...
أجب باللغة العربية فقط.`;
      ...
      // إنشاء رسالة افتراضية بشكل ذكي بناءً على اسم العميل
      let firstName = '';
      if (customerName) {
        // معالجة استخراج الاسم الأول بشكل أكثر ذكاءً
        const nameParts = customerName.split(' ');
        // تجنب أسماء قصيرة جداً قد تكون اختصارات
