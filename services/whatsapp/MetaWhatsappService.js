...
class MetaWhatsappService {
    ...
    /**
     * جلب معلومات جهة الاتصال من واتساب باستخدام رقم الهاتف
     * @param {string} phone رقم الهاتف (بدون + مثل: 218912345678)
     * @param {string} phoneNumberId معرف رقم الهاتف المستخدم للاتصال (اختياري)
     * @returns {Promise<object>} معلومات جهة الاتصال إذا تم العثور عليها
     */
    async getContactInfo(phone, phoneNumberId = null) {
        if (!this.initialized) {
            await this.initialize();
        }
        ...
        // التأكد من أن رقم الهاتف في الصيغة الصحيحة
        if (!phone) {
            throw new Error('رقم الهاتف غير محدد');
        }

        // إزالة علامة + من الرقم إذا وجدت
        ...
        try {
            // استخدام واجهة برمجة التطبيقات الخاصة بواتساب للحصول على معلومات جهة الاتصال
            // ملاحظة: هذه الميزة تتطلب توفر الصلاحيات المناسبة في حساب Meta
            const url = `${this.baseUrl}/${targetPhoneId}/contacts`;
            const headers = {
                'Authorization': `Bearer ${settingsToUse.config.accessToken}`,
                'Content-Type': 'application/json'
            };
            ...
            logger.info('MetaWhatsappService', 'تم الحصول على معلومات جهة الاتصال بنجاح', { 
                phone: formattedPhone, 
                responseData: response.data
            });
            
            // إذا كان هناك نتائج، نقوم بإرجاع أول جهة اتصال
            if (response.data && response.data.contacts && response.data.contacts.length > 0) {
                return response.data.contacts[0];
            } else {
                logger.warn('MetaWhatsappService', 'لم يتم العثور على معلومات لجهة الاتصال', { 
                    phone: formattedPhone 
                });
                return null;
            }
        } catch (error) {
            logger.error('MetaWhatsappService', 'خطأ في الحصول على معلومات جهة الاتصال', { 
                error: error.message,
                phone: formattedPhone,
                response: error.response?.data
            });
